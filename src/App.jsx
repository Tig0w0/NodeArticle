...
import {Plus,Search,X,Trash2,Upload,GripVertical,Link2,FileText,Music2,Image as ImageIcon,Map,FileCheck2} from 'lucide-react'
import './App.css'
...
function Graph({articles,setSelected}){const ordered=useMemo(()=>[...articles].sort((a,b)=>a.position.y-b.position.y),[articles]);const nodes=useMemo(()=>[...ordered.map((n,i)=>({...n,data:{...n.data,num:String(i+1).padStart(2,'0')}})),...evidence],[ordered]);const edges=useMemo(()=>{const main=ordered.slice(0,-1).map((n,i)=>({id:'m'+i,source:n.id,target:ordered[i+1].id,type:'smoothstep',className:'mainEdge'}));const refs=evidence.map((e,i)=>({id:'r'+i,source:e.side==='left'?e.id:e.target,target:e.side==='left'?e.target:e.id,type:'smoothstep',className:'refEdge',label:e.target==='lead'?'근거':e.target==='fact'?(i%2?'참고':'근거'):e.target==='quote'?'근거':e.target==='context'?'참고':'근거'}));return [...main,...refs]},[ordered]);return <section className="graphPanel"><div className="graphHead"><b>Article Graph</b><div className="graphTools"><button>↖</button><button>☝</button><button>100%</button><button>⌕</button><button>⛶</button><button className="addNode">노드 추가</button><button>⛶</button></div></div><div className="flowCanvas"><ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodeClick={(_,n)=>n.type==='article'&&setSelected(n.id)} defaultViewport={{x:92,y:-14,zoom:.74}} minZoom={.35} maxZoom={1.6} panOnDrag nodesDraggable={false}><Background variant="dots" gap={18} size={1}/><MiniMap pannable zoomable position="bottom-left"/><Controls position="top-right"/></ReactFlow></div></section>}
...
export default function App(){
 ...
 const [widths,setWidths]=useState([17.5,42.5,22.5,17.5]);const mins=[11,28,18,13]
 ...
 const reset=()=>setWidths([17.5,42.5,22.5,17.5])
 ...
 <main className="workspace">
  <div className="pane" style={{width:widths[0]+'%'}}><Sources/></div><div className="paneResizer" onPointerDown={e=>resize(0,e)} onDoubleClick={reset} title="드래그해서 패널 너비 조절 · 더블클릭으로 초기화"/>
